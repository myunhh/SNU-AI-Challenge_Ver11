...
관련 아이디어 중 **가장 강한 신호**다(참고: cross-frame contrast는 null, 어텐션 수확은
spearman +0.066로 약함 — 둘 다 `../CLAUDE.md`와 이 문서 §1 인용 자료 참고). 그래서
...
4. **본런 끝나면 in-sample 지표(마지막 체크포인트의 train subset 정확도 등)를 절대
   ...
   b. 그 후보들에 대해 **반드시 실제 test 819건 + `../grade/grade.py`로 채점**(자체
      답안키 v13 기준 est — `../CLAUDE.md`가 이미 경고했듯 est는 ≥1.5pp 격차만 신뢰,
      ±1pp는 실LB 슬롯 필요),
   c. **그 실측이 안전 하한(motion_weight=0 baseline, 즉 지금 Ver11 최선인 adapter_final,
      key v13 기준 est 0.9011)보다 실제로 나은지 확인한 뒤에만** "성공"이라고 보고할 것.
   `scripts/auto_pipeline.py`가 이미 이 (a)→(b) 패턴을 구현해뒀으니 그대로 재사용하거나
   `--motion-weight` 인자를 통과시키도록 최소 수정해서 쓸 것 — 새로 짜지 말 것.
