...
- **추론 중 VRAM 피크 감시(3090 이식성 판정)**: A100은 80GB라 OOM이 안 나지만, 최종 서빙
  후보 환경은 24GB급(3090/4090)이다. test 추론을 돌리는 동안 백그라운드에서
  `nvidia-smi --query-compute-apps=used_memory` 1~2초 폴링(또는 torch
  `max_memory_reserved`)으로 **피크 VRAM을 기록**하고, 피크가 **23GiB를 넘으면**(24GB 카드
  가용분 초과) — **추론은 중단하지 말고 끝까지 진행하되**, 완료 보고에 "**3090(24GB)에서는
  OOM 예상**"이라고 피크 수치와 함께 별도 항목으로 명시할 것. 참고 기준: 구 파이프라인
  (TTA3+cascade)의 4090 실측 피크는 19.9GiB였고, 신규 기본값(TTA4 + stage-2 always)은 이보다
  높아질 수 있어 이번 A100 런이 첫 실측이다.
...
OOM 여부 판정**(23GiB 초과 시 "3090에서는 OOM 예상"을 별도 항목으로) ⑥ 이상 징후 전부. 이후의 (선택) DPO
