...
**안전 하한 = 현 챔피언 = Ver8 DPO checkpoint-600 + 균형 TTA4, LB 0.91623**(2026-07-20 승격).
Ver11은 대체가 아니라 초과 도전 — **Ver11 제출이 이 값을 넘지 못하면 채택하지 않는다.**
(옛 문서에 남아있는 하한 0.90226(Ver4 ckpt1600)·0.90401(ckpt200)·0.91099(ckpt600+TTA3)와
"홀드아웃 쌍순서 ≳0.876" 바는 전부 과거 챔피언 기준이라 **무효**다.)

⚠️ 0.91099 → 0.91623은 **가중치가 바뀐 게 아니라 서빙 TTA 세트만 TTA3→균형 TTA4로 바꾼 결과**다
(+0.524pp). 따라서 warm-start용 어댑터는 그대로 ckpt600 하나뿐이며, "TTA4 어댑터" 같은 건
존재하지 않는다. Ver11은 이미 균형 TTA4가 추론 기본값(`--tta 4`)이라 추가 조치 불필요.
...
- **warm-start 어댑터 = Ver8 DPO checkpoint-600 (현 챔피언 가중치, 이 가중치의 LB = TTA4 서빙 시
  0.91623 / 구 TTA3 서빙 시 0.91099).** 아래 07-16 절의 ckpt200(0.90401) 언급은 당시 챔피언
  기준의 옛 기록이다. 같은 DPO 궤적이라 LoRA 구성 동일
